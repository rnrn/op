...
/op-story-finder --audit               # audit the backlog against its own declared spec
```

**`--audit` — backlog-conformance mode** (the family audits code, docs, and debt, but the
backlog itself is a corpus too): run the canonical helper over the declared docs root —
`node ~/.claude/scripts/lib/unit-status.mjs --root <repo> --dir <docs-root> --check
--convention <declared>` — and report (a) units whose status is not where the declaration
says, (b) the full status-token vocabulary with UNDECLARED tokens called out (pass the
project-declared extras via `--tokens-closed`/`--tokens-open`), (c) ambiguous bodies
(2+ closed tokens). Every finding names the unit heading + file. This is read-only —
fixing the corpus belongs to the user or an op-watch campaign, never to the audit itself.
...
2. **Parse matches** (heading/field shapes vary by layout — handle all). **Never hand-roll status
   parsing** — resolve each unit's status with the canonical helper `node ~/.claude/scripts/lib/unit-status.mjs`
   `--root <repo> [--dir <docs-root>]` (heading token → body `Status:` line → body scan, with
   ambiguity detection; falls back to the copy shipped in the bundle's `scripts/lib/`). An ad-hoc
   parser against a mixed-convention corpus miscounts open work (field-verified ~2x overstatement).
   When `--check --convention <declared>` shows the corpus violating its own declaration, report
   the mismatch as a finding next to the results — never silently pick one convention:
