...
3. **Frame the required inputs** — User Spec (outcome, constraints, non-goals, success signal; `N/A - reason` only for internal cleanup), Contract, Runtime rung, Owner subsystem, Config source of truth, Scope, Invariants, Risk list, PR/security risk, Proof path. Any missing -> stop and make it concrete before coding. **Resolve every path the User Spec names against the repo BEFORE scoping** — a path that does not exist, or is a thin re-export shim (`sys.modules[__name__] = _impl`, `from x import *`, a few-line file re-exporting a real module elsewhere), is itself a finding: follow it to the real owner module and scope THAT, don't gate the shim. **Boundary changes search the decision log first**: before planning a change to an access model, ownership rule, protocol, or architectural boundary, search the project's ADR/decision log for an existing decision on it — "no decision exists" is a conclusion to earn, not assume (field: a task's first draft would have rewritten a settled ADR past the process). Mechanical gates the project declares (in `AGENTS.md`, `docs/HANDBOOK.md`, or feedback rules — e.g. build checks, file-budget ratchet, lint gates) that apply to the touched files belong in the Proof path; the gate's thresholds stay project-local. **Timing surfaces need live-proof:** when the change touches concurrency, event ordering, or any seam where a mock replaces a slow operation, a green test suite is NOT the proof — plan a live run for that seam (the mock that removes the delay removes the race window with it). The Proof path must target the User Spec's **success signal (the outcome)**, not a stand-in: when only a **proxy** is runnable here (a local smoke vs a real-scale / remote / user-environment outcome), name what the proxy covers and the gap that remains to the real outcome — a passing proxy scores `WATCH` for that item, not `PASS`. **Stack-deviation gate:** when the change creates code files, apply the stack-deviation gate — **read `references/stack-gate.md`** (run `~/.claude/scripts/lib/stack-check.mjs`; a non-declared language is a `WATCH` + human-accepted `### Decision`; an undeclared stack turns the guard off → `WATCH` + declare first; a model drafts `status: pending` and hands off, never self-accepts).
4. **Select feedback rules** — read `docs/feedback/index.md`, then `baseline.md`. Map planned files/services/runtimes/docs/skills/agents to feedback files via the index; open only baseline plus those and apply each file's `Checklist (for preflight)`. **A rule without a mechanism describes a boundary, it does not hold it** (field: two auth rules lived a month while the code violated both — zero tests behind them): for each applied rule, name what ENFORCES it (test, lint, guard, script) — a rule with no named mechanism is applied as `declarative` and must not be scored as a guarantee. **Respect rule dates**: a rule whose introduction postdates the work under review was DERIVED FROM that work, not violated by it — never retro-blame. Conflicts: index priority order, else narrowest project-local rule + `WATCH`. Workflow/skill/docs/prompt/agent tasks also load `rules-and-skills.md` and `process.md`. **Test-touching tasks** — any change to tests, fixtures, the test DB/bootstrap, the E2E harness, or that adds/relaxes assertions or skips — **must state a `Test Plan` BEFORE writing any test code**: `{language, framework, isolation, run-cmd}`. The plan's `language` is stack-checked against the declared Stack-Profile language(s) (and, under op-watch, against the campaign's frozen `test_language` state field) — a non-declared test language scores `WATCH` and is a **plan-time `### Decision`** (accept e.g. Python tests in a Go repo only with a recorded rationale + isolated dir, or write the suite in the declared language), decided *before* authoring a throwaway suite rather than caught on output. Then also load `docs/feedback/test-infra.md` (`TI1-TI10`) plus, **only if the `docs/testing/` directory exists**, `docs/testing/testing-rules.md` and `docs/testing/e2e-maintenance-guide.md`, and apply their `Checklist (for preflight)`. **When the task is test-touching and `docs/testing/` does NOT exist, say so — that guard is OFF and it scores `WATCH`** ("testing contract undeclared — recommend `docs/testing/testing-rules.md` + `e2e-maintenance-guide.md`"), exactly like an undeclared stack: silence hides that the project's testing lore lives only in someone's memory. For non-test tasks the absent dir stays silently skipped; a test-infra violation (shared/non-isolated state, hardcoded seed IDs, fragile selectors, bare-mock-for-typed-dep, mocking owned services, weakened assertions) is a gate, not a nuance. Project-local feedback only. Never scan git history — that is `op-feedback-harvest`, on explicit request only.
