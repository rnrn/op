...
3. **Frame the required inputs** — User Spec (outcome, constraints, non-goals, success signal; `N/A - reason` only for internal cleanup), Contract, Runtime rung, Owner subsystem, Config source of truth, Scope, Invariants, Risk list, PR/security risk, Proof path. Any missing -> stop and make it concrete before coding. Mechanical gates the project declares (in `AGENTS.md`, `docs/HANDBOOK.md`, or feedback rules — e.g. build checks, file-budget ratchet, lint gates) that apply to the touched files belong in the Proof path; the gate's thresholds stay project-local.
