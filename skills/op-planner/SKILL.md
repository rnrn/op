...
Checkpoint git policy: `PLANNER_CHECKPOINT.md` is a single-slot scratch draft overwritten by
each run — add it to the project's `.gitignore` and never commit it; the
durable output is the epic/story files written under `--apply`.
...
/op-planner add <epic-ref> <task> [--apply]   # Append ONE unit to an existing epic (light path)
```

**`add` — the single-unit append path.** The most common action in a mature repo is adding one
task to an existing epic, and the full checkpoint cycle is heavy enough that operators bypass
the skill and hand-edit the epic — losing the duplicate check and status discipline (field
measurement: five hand-written tasks in three epics, zero skill invocations). For `add`, the
CONVERSATION is the checkpoint: run the op-story-finder duplicate check, pick the next free
number, place the status per the declared convention, and show the exact unit text; with
`--apply` in the same request, append exactly that one unit section to the existing epic file —
no `PLANNER_CHECKPOINT.md` for single-unit appends. Multi-unit or new-epic requests always take
...
3. **Check for duplicates AND target-existence** — search existing stories for the
   ...
   **Status placement & parsing:** write each new unit's status where the declared convention
   keeps it (flat-epic/tasks layouts: in the unit HEADING); read existing statuses only via the
   canonical helper `node ~/.claude/scripts/lib/unit-status.mjs` — never a hand-rolled regex. If its `--check` shows the
   corpus violating its own declaration, surface that in the checkpoint instead of adapting silently.
