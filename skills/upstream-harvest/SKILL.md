...
Checkpoint git policy: `UPSTREAM_HARVEST_CHECKPOINT.md` is a single-slot scratch draft overwritten by
each run — add it to the project's `.gitignore` and never commit it; the
durable output is the docs/tests written under `--apply`.
