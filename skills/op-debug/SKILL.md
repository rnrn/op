...
**Mock-latency heuristic:** a green test with a red live run points at the MOCK before the code — a mock that replaces a slow operation (DB write, network, sleep) removes the delay AND the race window with it, so the suite goes blind to timing defects silently. When live behavior contradicts a green suite, first ask which mock swallowed the window.
