...
2. **Dispatch** the next sub-step deterministically from state. **Priority: a `built`/`fixed` unit
   ...
   - unit `fixed`/`built`, unconfirmed → **verify, deterministic-first:**
     ...
     2) **completion-claim check** when the unit carries a done-report with test/green claims: dispatch `op-closeout` — proof triple (command + SHA + result), red named by nodeid, base-commit classification, live-proof for timing seams — before `verified`.
     3) **scoped semantic audit** for a HIGH-risk unit or at closure: `op-audit verify --scope
