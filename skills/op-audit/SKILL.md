...
3. **Run the engine.** Invoke the **Workflow** tool with `scriptPath` =
   ...
   **If the Workflow tool is unavailable, fall back to sequential `Task` agents — one per lane**
   ...
   the discovery root) + REUSE the `knownFindings` ids + the honest-limits line ("state plainly what your result does NOT cover, even when green" — the uncovered list is part of the deliverable) + the provenance rule (every finding carries a REPRO COMMAND, and the report separates "personally reproduced" from "reported by a tool/agent" — field: an AST guard once produced 5 phantom defects that a live check dissolved; a red claimed as a regression is classified against the base commit first: `git worktree add --detach <tmp> <base>`, same run, compare, remove); **output = the ledger `findings[]` shape**
