...
## Declared purpose (the check the validator cannot run)

Every map carries `<!-- map-purpose: ... -->` near the top: one sentence naming the artifact's
primary job and what is secondary to it. Judge every edit against this sentence first — the
regexes prove the panels/chips/tooltips are CONSISTENT, only the purpose can prove they are
RIGHT (a consistent backlog import once ate ~45% of every panel with filter chips). If an edit
grows a secondary element at the expense of the primary one, it is a regression regardless of a
green validator.
