...
// Ported and generalized from the origin project's python map validator: the map is a VERIFIED view —
...
// 0. declared purpose — the check the regexes cannot run (a consistent edit can still be wrong);
// note-level so pre-existing artifacts stay green, but new maps should carry it (design-language.md)
const purpose = doc.match(/<!--\s*map-purpose:\s*([\s\S]*?)-->/);
notes.push(purpose ? `purpose declared: ${purpose[1].trim().slice(0, 80)}` : "no declared purpose — add <!-- map-purpose: ... --> (see design-language.md)");
