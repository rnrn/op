...
- **Grep the project's contract docs for stale skill names** (`AGENTS.md`,
  `CLAUDE.md`, `docs/**`): bare names like `/preflight` or `/review` that match
  no installed skill are the single most likely stale reference after install
  (field case: five files pointed at a gate that could not be invoked). Fix
  them to the real names (`op-preflight`, ...) or delete them.
